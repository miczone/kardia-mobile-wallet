...
export const getFadoTotalStakedAmount = async (smcAddress: string) => {
  try {
    kardiaContract.updateAbi(FADO_STAKING_ABI);
    const rs = await kardiaContract
      .invokeContract('totalStakedAmount', [])
      .call(smcAddress);
...
export const stakeFadoToken = async (stakeAmount: number, wallet: Wallet) => {
  ...
  const approveStatus = await kardiaContract
    .invokeContract('approve', [FADO_STAKE_SMC, stakeAmount])
    .call(FADO_TOKEN_SMC);
  try {
    if (approveStatus === true) {
      ...
      if (wallet !== undefined && wallet.privateKey !== undefined) {
        ...
        const txtAddress = kardiaContract
          .invokeContract('stake', [stakeAmount])
...
export const stakerInfo = async (stakerAddress: string) => {
  ...
  if (stakerAddress !== undefined) {
    const resObj = await kardiaContract
      .invokeContract('stakerInfo', [stakerAddress])
      .call(FADO_STAKE_SMC);
    return resObj;
  }
  return '';
